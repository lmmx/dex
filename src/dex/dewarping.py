...
from .path_utils import dewarped_path, has_been_dewarped
...
def dewarp_images(book: Book) -> None:
    ...
    """
    item_images_to_dewarp = [p for p in book.images if not has_been_dewarped(p)]
    ...
    dewarped_images = sorted(  # noqa: F841
        [dewarped_path(p) for p in book.images if has_been_dewarped(p)],
    )
    unfixed = [p for p in book.images if not has_been_dewarped(p)]
    if any(unfixed):
    ...
    # book.scanned = None  # Removed LayoutLMv3 capabilities here
    return
